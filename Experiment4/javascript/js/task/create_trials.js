...
/* item-context association trials */
let asso_test = [1, 5, 4, 8, 7, 11, 10, 14, 13, 17, 16, 20, 19, 23, 22, 26, 25, 29, 28, 32, 31, 35]
// function that gets the color of the image
function get_pair(trial, LeftRight, block) {

    object = trials[block][trial].stim
    ...
    if (trial < 6) {
        ...
        which_lure = shuffle([-6, 6])[0]
    }

    if (LeftRight == "left") {
        left_context = trials[block][trial].event
        right_context = trials[block][trial + which_lure].event
    } else if (LeftRight == "right") {
        left_context = trials[block][trial + which_lure].event
        right_context = trials[block][trial].event
    }
...
for (let block = 0; block < 11; block++) {
    let asso_subArray_trials = []
    asso_test = shuffle(asso_test)

    for (let trial = 0; trial < asso_test.length; trial++) {
        LeftRight = shuffle(["left", "right"])[0]
        let displays = get_pair(asso_test[trial], LeftRight, block)

        asso_subArray_trials.push(new Data_row({
            time_end: "incomplete",
            practice: block == 0 ? true : false,
            association: true,
            block: block,
            trial: trial,
            answer: LeftRight,
            object: displays.object,
            left_context: displays.left_context,
            right_context: displays.right_context
        }))
    }
    asso_trials.push(asso_subArray_trials)
}
...
// memory
let mem_trials = []
let contextCondition = shuffle(["present", "absent"])[0]
let context_type = shuffle([...Array(11).fill(contextCondition)])
...
for (let block = 0; block < 11; block++) {
    ...
    for (let trial = 0; trial < mem_test.length; trial++) {
        ...
        mem_subArray_trials.push(new Data_row({
            ...
            trial: trial,
            answer: LeftRight,
            context_type: context_type[block],
